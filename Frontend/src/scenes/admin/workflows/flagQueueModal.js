...
const MODAL_KEY = 'flagQueueModal';

export async function openFlagQueueModal(scene) {
  if (scene[MODAL_KEY]) {
    showToast(scene, 'Flag queue is already open.');
    return;
  }
  ...
  try {
    ...
    showToast(scene, getApiErrorMessage(error, 'Unable to load content flags'));
    return;
  }

  renderFlagQueueModal(scene, rows || []);
}

export function destroyFlagQueueModal(scene) {
  destroySceneModal(scene, MODAL_KEY);
}

function renderFlagQueueModal(scene, rows) {
  ...
  const rowHtml = sortedRows.map((row) => {
    const flagId = escapeHtml(row?.contentFlagId || '');
    const contentId = escapeHtml(row?.content?.contentId || 'Unknown');
    const title = escapeHtml(row?.content?.title || 'Untitled');
    const topicName = escapeHtml(row?.content?.topic?.topicName || 'Unknown Topic');
    const reportedBy = escapeHtml(
      row?.reportedBy?.username ||
      row?.reportedBy?.learnerId ||
      'Unknown learner'
    );
    const createdAt = escapeHtml(formatDate(row?.createdAt));
    const reason = escapeHtml(row?.reason || 'UNKNOWN');
    const details = escapeHtml(row?.details || 'No additional details.');
  ...
  actionButtons.forEach((button) => {
    button.addEventListener('click', async (event) => {
      ...
      const notesEl = modal.querySelector(`[data-flag-notes="${escapeCssSelector(flagId)}"]`);
      const resolutionNotes = notesEl?.value?.trim() || '';
      const status = action === 'review' ? 'REVIEWED' : 'DISMISSED';

      if (status === 'DISMISSED' && !resolutionNotes) {
        setFlagQueueStatus(scene, 'Resolution notes are required to dismiss a flag.', '#ffc7c7');
        return;
      }
      ...
      setFlagActionButtonsEnabled(scene, false);
      ...
      try {
        ...
        const rowEl = modal.querySelector(`[data-flag-row-id="${escapeCssSelector(flagId)}"]`);
        rowEl?.remove();
        ...
        updateFlagPendingCount(scene, remaining);
        setFlagQueueStatus(scene, 'Flag updated.', '#a7f0c2');
        showToast(scene, status === 'REVIEWED' ? 'Flag marked reviewed.' : 'Flag dismissed.');
      } catch (error) {
        setFlagQueueStatus(scene, getApiErrorMessage(error, 'Flag action failed'), '#ffc7c7');
      } finally {
        setFlagActionButtonsEnabled(scene, true);
      }
    });
  });
}

function updateFlagPendingCount(scene, count) {
  const summary = scene.flagQueueModal?.querySelector('#flag-queue-count');
  if (summary) {
    summary.textContent = `Total open flags: ${count}`;
  }
}

function setFlagQueueStatus(scene, message, color = '#ffd4a6') {
  const statusEl = scene.flagQueueModal?.querySelector('#flag-queue-status');
  if (!statusEl) return;
  statusEl.textContent = message;
  statusEl.style.color = color;
}

function setFlagActionButtonsEnabled(scene, enabled) {
  const buttons = scene.flagQueueModal?.querySelectorAll('button[data-flag-action]');
  const textareas = scene.flagQueueModal?.querySelectorAll('textarea[data-flag-notes]');
